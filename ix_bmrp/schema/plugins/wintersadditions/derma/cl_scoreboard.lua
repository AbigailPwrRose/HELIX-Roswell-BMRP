...
function PANEL:Init()
	...
	self.class:DockMargin(5, 0, 0, 0)
	self.class:Dock(TOP)
	self.class:SetTextColor(color_white)
	self.class:SetFont("ixSmallFont")
...
function PANEL:Update()
	...
	local description = hook.Run("GetCharacterDescription", client) or (client:GetCharacter() and client:GetCharacter():GetDescription()) or ""
	local class = ix.class.Get(client:GetCharacter():GetClass()).name or ""
  
    local rankDisplay = ""
    local rank = client:GetCharacter():GetRank()
    local factionData = ix.faction.Get(client:GetCharacter():GetFaction())
    if factionData and factionData.Ranks and factionData.Ranks[rank] then
        rankDisplay = " | "..factionData.Ranks[rank][1]
    end
  
    local fakeJob = client:GetCharacter():GetFakejob()
    if fakeJob != "" then
      fakeJob = " | "..fakeJob 
    end
	...
	if (self.class:GetText() != class) then
		self.class:SetText("("..class..fakeJob..rankDisplay..")")
