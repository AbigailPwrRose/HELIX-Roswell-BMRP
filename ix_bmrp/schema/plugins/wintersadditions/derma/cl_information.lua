...
function PANEL:Init()
	...
	if (!suppress.characterInfo) then
		...
		if (!suppress.rankTime) then
			self.rankTime = self.characterInfo:Add("ixListRow")
			self.rankTime:SetList(self.characterInfo.list)
			self.rankTime:Dock(TOP)
		end
...
function PANEL:Update(character)
	...
	if (self.rank) then
		self.rank:SetLabelText("Rank")
		self.rank:SetText(faction.Ranks[rank][1])
		self.rank:SizeToContents()
	end
  
	if (self.rankTime) then
        local Timer = faction.Ranks[rank][5]
        local TimerScale = "Seconds"
        Timer = Timer - character:GetRankTime()
        if Timer >= 60 then 
            Timer = Timer / 60
            TimerScale = "Minutes"
        end
		...
		self.rankTime:SetLabelText("Rank Timer")
		self.rankTime:SetText(math.Round( Timer, 2 ).." "..TimerScale.." till promotion")
		self.rankTime:SizeToContents()
	end
