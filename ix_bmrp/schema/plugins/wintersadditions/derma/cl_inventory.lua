...
function PANEL:ExtraPaint(width, height)
	...
	if (itemTable and itemTable.PaintExtra) then
		itemTable.PaintExtra(self, itemTable, width, height)
	end
end
...
function PANEL:Paint(width, height)
	...
	if (itemTable and itemTable.Paint) then
		itemTable.Paint(self, itemTable, width, height)
    	self:ExtraPaint(width, height)
	else
        local RareList = {
          ["Common"] = Color(60,60,60,85),
          ["Uncommon"] = Color(203,196,0,85),
          ["Rare"] = Color(111,200,0,85),
          ["Mythic"] = Color(25,90,255,90),
          ["Sidereal"] = Color(41,138,187,120),
          ["Legendary"] = Color(32,54,216,90),
          ["Xen"] = Color(126,6,255,160),
        }
        if itemTable.rarity then
        	surface.SetDrawColor(60, 60, 60, 85)
        	surface.DrawRect(2, 2, width - 4, height - 4) 
        	surface.SetDrawColor(RareList[itemTable.rarity])
        	surface.DrawRect(6, 6, width - 12, height - 12) 
        else
        	surface.SetDrawColor(60, 60, 60, 85)
        	surface.DrawRect(2, 2, width - 4, height - 4) 
        	surface.SetDrawColor(Color(60,60,60,85))
        	surface.DrawRect(6, 6, width - 12, height - 12) 
        end
    end
end
...
function PANEL:OnTransfer(oldX, oldY, x, y, oldInventory, noSend)
	...
	if (inventory) then
		...
		if (hook.Run("CanTransferItemUpdated", item, inventories[oldInventory.invID], inventories[self.invID]) == false) then
			return false, "notAllowed"
		end
		if (item.CanTransfer and
			item:CanTransfer(inventory, inventory != inventory2 and inventory2 or nil) == false) then
            print(item:CanTransfer(inventory, inventory != inventory2 and inventory2 or nil) == false)
			return false
		end
	end
