...
function ix.inspect.MakeParagraph(V,Dt) 
    table.insert(Dt,V*50,"\n")
    if V >= 1 then
        V = V - 1
        Dt = ix.inspect.MakeParagraph(V,Dt) 
    end
    return Dt
end
...
if SERVER then
    ...
    util.AddNetworkString("InspectEnt_SetDescription")
    ...
    net.Receive("InspectEnt_SetName", function(len, ply)
 	...
 	net.Receive("InspectEnt_SetDescription", function(len, ply)
        if !ply:IsAdmin() then return end
        local ent = net.ReadEntity()
        local Desc = net.ReadString()

        if !IsValid(ent) or ent:GetClass() != "ix_inspectable" then return end
        ent:SetCustomDescription(Desc)
    end)
    ...
    net.Receive("InspectEnt_Start", function()
        ...
        if inspectEnt:GetClass() == "ix_inspectable" then
            ...
            description = inspectEnt:GetCustomDescription()
        end
        ...
        inspectPanel.Paint = function(s, w, h)
            ...
            if fraction > 0.01 and animState == 2 then
                if name then
                    draw.SimpleText(name, "Trebuchet24", w * 0.03 , h * 0.1, Color(255, 255, 255, math.min(255, fraction * 255 * 2)), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
                end
                if description then
                    local DT = string.ToTable(description)
                    local Loops = math.floor(table.maxn(DT) / 50)
                    if Loops >= 1 then
                            DT = ix.inspect.MakeParagraph(Loops,DT) 
                    end
                    local DescFixed = ""
                    for k,v in pairs(DT) do
                            DescFixed = DescFixed .. v
                    end
                    draw.DrawText( DescFixed, "DefaultFixed", w * 0.03, h  * 0.14, color_white, TEXT_ALIGN_LEFT )
                end
    ...
    net.Receive("InspectItem_Start", function()
        ...
        local description = inspectItem and inspectItem.GetName and inspectItem:GetDescription()
        inspectPanel.Paint = function(s, w, h)
            -- Compute time interpolation
            local fraction = 0
            if animState == 1 then fraction = math.ease.OutCubic(animProgress / animDuration)
