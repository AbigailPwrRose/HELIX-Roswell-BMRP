...
ENT.PrintName = "Military PCV" -- The name that will appear in the spawn menu.
...
function ENT:Initialize()
  ...
  self:SetModel( "models/opfor/items/w_suit.mdl" ) -- Sets the model for the Entity.
  ...
  if IsValid(client) and client:IsPlayer() then
    ...
    if PreWear == 0 then
      if !(self:GetMaterial() == "models/props_combine/stasisshield_sheet") then
          ...
          client:ChatNotify("You have equipped a PCV")
...
function ENT:OnPopulateEntityInfo(container)
	...
	name:SetImportant()
	name:SetText("Military PCV")
	name:SizeToContents()
	local description = container:AddRow("description")
	description:SetText("A military vest thats specialised to be more resistant.")
