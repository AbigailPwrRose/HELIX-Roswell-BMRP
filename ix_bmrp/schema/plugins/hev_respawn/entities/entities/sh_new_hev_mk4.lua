...
ENT.PrintName = "Mark 4 HEV suit" -- The name that will appear in the spawn menu.
...
function ENT:Initialize()
  ...
  self:SetModel( "models/player/hlew/extras/hev_scientist_empty.mdl" ) -- Sets the model for the Entity.
  ...
  if IsValid(client) and client:IsPlayer() then
    ...
    if PreWear == 0 then
      if !(self:GetMaterial() == "models/props_combine/stasisshield_sheet") then
          ...
          Character:SetData("WearingHEVMK4",1)
          client:ChatNotify("You have equipped a HEV Mk4")
          self:SetMaterial("models/props_combine/stasisshield_sheet")
          timer.Simple( 45, function() 
              self:SetMaterial("") end )
      end 
    else
      local KitType = Character:GetData("WearingHEVMK4",0)
      if KitType == 1 then
        self:SetMaterial("")
...
function ENT:OnPopulateEntityInfo(container)
	...
	name:SetImportant()
	name:SetText("Hazardous EnVironment Suit MK4")
	name:SizeToContents()
	local description = container:AddRow("description")
	description:SetText("A state of the art, all new, Hazardous EnViroment Suit MK4. Finally out of production, these bad-boys are the cream de la cream of modern HEV tech! Although, it is slightlt less protective than the previous version...")
