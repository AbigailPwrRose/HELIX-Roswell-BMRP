	...
	function PLUGIN:PlayerNoClip(client, state)
		if (hook.Run("CanPlayerEnterObserver", client)) then
			if (state) then
				...
				if pac then pac.TogglePartDrawing(client, false) end
				local playercont,playerid = client,client:UserID()
				do
					timer.Create( "observer_pac3hidden" .. playerid, 0.5, 0, function()
						if IsValid(playercont) and pac then return pac.TogglePartDrawing(playercont, false) end -- We REALLY hate pac3 drawing while noclipped.
						timer.Remove( "observer_pac3hidden" .. playerid ) -- Pac3 doesn't exist in this scenario, no need to waste operations.
					end)
				end
				...
				if pac then pac.TogglePartDrawing(client, true) end
				timer.Remove( "observer_pac3hidden" .. client:UserID() ) -- Stop hiding pac3
