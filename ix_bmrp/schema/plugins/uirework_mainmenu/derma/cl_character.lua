surface.CreateFont( "SpecialFont_Gibbous", {
	font = "HudDefault", 
	extended = false,
	size = 35,
	weight = 1000,
})
surface.CreateFont( "Font_RoswellTitleFont", {
	font = "ixTitleFont", 
	extended = false,
	size = 120,
	weight = 500,
})
surface.CreateFont( "Font_RoswellSubTitleFont", {
	font = "ixTitleFont", 
	extended = false,
	size = 60,
	weight = 220,
...
-- character menu main button list Horizontal Edition
PANEL = {}

function PANEL:Init()
	local parent = self:GetParent()
	self:SetSize(parent:GetWide() * 0.25, ScrH()/2)
    self:DockMargin(20,25,25,0-ScrH()*0.06)
	self:GetVBar():SetWide(0)
	self:GetVBar():SetVisible(false)
end
...
vgui.Register("ixCharMenuButtonListHorizontal", PANEL, "DScrollPanel")
...
function PANEL:Init()
	...
	logoPanel:SetSize(ScrW(), ScrH())
	logoPanel:SetPos(0, 0)
	logoPanel.Paint = function(panel, width, height)
        ...
        -- Textures main menu bg
    	surface.SetDrawColor( 255, 255, 255, 255 ) 
		surface.SetMaterial(Material("BMRPMainMenuTexture.png", "noclamp smooth"))
    ...
    titleLabel:SetFont("Font_RoswellTitleFont")
    titleLabel:SetText("Site Roswell BMRP")
    titleLabel:SizeToContents()
    titleLabel:SetPos(halfWidth - titleLabel:GetWide() * 0.6, ScrH()*0.73)
    titleLabel:SetPaintedManually(true)
    local SubTitle = self:Add("DLabel")
    SubTitle:SetTextColor(color_black)
    SubTitle:SetFont("Font_RoswellSubTitleFont")
    SubTitle:SetText("Anti-Mass Spectrometer Update")
    SubTitle:SizeToContents()
    SubTitle:SetPos(halfWidth - SubTitle:GetWide() * 0.6, ScrH()*0.82)
    SubTitle:SetPaintedManually(true)

	-- button list
	self.mainButtonList = self:Add("ixCharMenuButtonListHorizontal")
	self.mainButtonList:Dock(LEFT)

	-- create character button
	local createButton = self.mainButtonList:Add("ixMenuButton")
    createButton:SetFont("SpecialFont_Gibbous")
	createButton:SetText("create")
    createButton:SetTextColor(color_black)
    ...
    self.loadButton:SetFont("SpecialFont_Gibbous")
	self.loadButton:SetText("load")
    self.loadButton:SetTextColor(color_black)
	...
	if (extraURL != "" and extraText != "") then
        ...
        extraButton:SetFont("SpecialFont_Gibbous")
        extraButton:SetTextColor(color_black)
    ...
    self.returnButton:SetFont("SpecialFont_Gibbous")
    self.returnButton:SetTextColor(color_black)
	...
	infoLabel:SetTextColor(Color(0, 0, 0, 225))
