...
function PLUGIN:AreaThink()
	for _, client in player.Iterator() do
		...
		if (ix.area.stored[client.ixArea]) and (ix.area.stored[client.ixArea].type == "xen_area") and !((client:GetMoveType() == MOVETYPE_NOCLIP) or (character:GetData("WearingHEVStuff",0) == 1) or (client:GetFactionName() == "Xenian")) then
            client:XenPoison(5)
        elseif (ix.area.stored[client.ixArea]) and (ix.area.stored[client.ixArea].type == "ams_radioactive") 
      and !((client:GetMoveType() == MOVETYPE_NOCLIP) or (character:GetData("WearingHEVStuff",0) == 1) 
        or (client:GetFactionName() == "Xenian")) then
            client:XenPoison(1)
        end
	end
end
