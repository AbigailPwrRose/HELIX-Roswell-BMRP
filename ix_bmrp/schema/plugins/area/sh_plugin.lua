...
ix.area.fancyDisplayList = {
  ["DefaultTo"] = "Unknown",
  ["Lambda_"] = "Lambda Core",
  ["Dorms_"] = "Dormitories",
  ["Topside"] = "Topside",
  ["Biodome_"] = "Biodome",
  ["Maintenance_"] = "Maintenance Tunnels",
  ["Tram_"] = "Tram Network",
  ["Trams_"] = "Tram Network",
  ["SecA_"] = "Sector A",
  ["SecC_"] = "Sector C",
  ["SecSec_"] = "Security Sector",
  ["MaintZ_"] = "Logistics Zone",
  ["Garrage_"] = "Garage",
  ["Residue_"] = "Residue Processing",
  ["Barracks_"] = "Nat. Guard Barracks",
  ["Offices_"] = "Offices",
  ["Cliffs_"] = "Cliffs",
  ["Xen_"] = "Xen; The Border World",
  ["AMS_"] = "Anti-Mass Spectrometer",
  ["SecG_"] = "Sector G | Radio Center",
}
...
function PLUGIN:SetupAreaProperties()
	...
	ix.area.AddType("xen_area")
	ix.area.AddType("ams_radioactive")
...
do
    ...
    function PLAYER:ConfiguredGetMesaArea()
		local Dirty = self.ixArea
        local CleanedUp = ix.area.fancyDisplayList["DefaultTo"]
        if (Dirty) then
          for k,v in pairs(ix.area.fancyDisplayList) do
            if string.StartsWith(Dirty, k) then 
              CleanedUp = v
            end
          end
          if string.EndsWith(Dirty, "Checkpoint") then
              CleanedUp = "Security Checkpoint"
          end
        end
        return CleanedUp
	end
