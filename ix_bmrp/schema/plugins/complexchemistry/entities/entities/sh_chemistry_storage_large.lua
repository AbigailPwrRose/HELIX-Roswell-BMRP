...
ENT.PrintName = "Chemical Storage Tank"
ENT.Author = "Winter"
ENT.Spawnable = true
ENT.AdminSpawnable = true
ENT.Model = "models/props_industrial/chloride_tank01.mdl"
ENT.MaxVol = 5000
...
function ENT:OnPopulateEntityInfo(container)
  ...
  local name = container:AddRow("name")
	name:SetImportant()
	name:SetText("Chemical Tank")
  if (ChemTable[1][1][1] != 0) and (ix.chemistry.List[ChemTable[1][1][1]]) then
    name:SetText("Chemical Tank of "..ix.chemistry.List[ChemTable[1][1][1]][1])
  end
  name:SetBackgroundColor(Color(230,230,230))
	name:SizeToContents()
	local description = container:AddRow("description")
	description:SetText("A large chemical tank, capable of holding 5 litres of chemicals.")
	description:SizeToContents()
  local vol = container:AddRow("Contains")
  vol:SetBackgroundColor(Color(200,200,200,190))
  vol:SetFont("DermaDefault")
    if (ChemTable[1][1][1] != 0 ) and (ix.chemistry.List[ChemTable[1][1][1]]) then
      vol:SetText("This chemical tank contains ".. ChemTable[1][1][2] .."ml of "..ix.chemistry.List[ChemTable[1][1][1]][1])
    else
      vol:SetText("This chemical tank is empty") 
    end
  vol:SizeToContents()
end