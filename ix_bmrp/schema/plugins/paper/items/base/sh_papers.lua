...
ITEM.description = "A journal with a blue cover and a pen."
...
function ITEM:ReadWrite(id,client) 
    if (id) and (client != nil) then
        netstream.Start(client, "receivePaper", id, self:GetData("PaperData") or "")
    end
    return false
end
