...
PLUGIN.name = "Material Panels"
PLUGIN.author = "Winter Rose"
PLUGIN.description = "Adds material panels that can be placed on the map."

-- List of available panel dislays.
PLUGIN.list = PLUGIN.list or {}

if (SERVER) then
	util.AddNetworkString("ixMPanelList")
	util.AddNetworkString("ixMPanelAdd")
	util.AddNetworkString("ixMPanelRemove")
	...
	function PLUGIN:AddMPanel(position, angles, url, scale, brightness)
	...
	function PLUGIN:RemoveMPanel(position, radius)
	...
	local function CacheMaterial(index)
		...
		local info = PLUGIN.list[index]
			local material = Material(info[6], "noclamp smooth")
	...
	local function UpdateCachedPreview(url)
		...
		local filename = exploded[#exploded]
			local preview = Material(url, "noclamp smooth")
	...
	net.Receive("ixMPanelAdd", function()
	...
	net.Receive("ixMPanelRemove", function()
		local index = net.ReadUInt(32)
	...
	net.Receive("ixMPanelList", function()
		...
		timer.Create("ixMCache3DPanels", 1, #CacheQueue, function()
			if (#CacheQueue > 0) then
				CacheMaterial(CacheQueue[1])

				table.remove(CacheQueue, 1)
			else
				timer.Remove("ixMCache3DPanels")
			end
		end)
	end)
	...
	function PLUGIN:PostDrawTranslucentRenderables(bDrawingDepth, bDrawingSkybox)
		...
		-- Panel preview
		if (ix.chat.currentCommand == "mpaneladd") then
			self:PreviewPanel()
		end
...
ix.command.Add("MPanelAdd", {
	...
	OnRun = function(self, client, url, scale, brightness)
		...
		-- Add the panel.
		PLUGIN:AddMPanel(position + angles:Up() * 0.1, angles, url, scale, brightness)
		return "@panelAdded"
	end
})

ix.command.Add("MPanelRemove", {
	...
	OnRun = function(self, client, radius)
		...
		local amount = PLUGIN:RemoveMPanel(position, radius)
