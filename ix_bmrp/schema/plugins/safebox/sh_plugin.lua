...
ix.config.Add("safeInvWidth", 8, "How many slots in a row there is in a safebox inventory.", nil, {
	data = {min = 0, max = 20},
	category = PLUGIN.name
})

ix.config.Add("safeInvHeight", 12, "How many slots in a column there is in a safebox inventory.", nil, {
	data = {min = 0, max = 20},
	category = PLUGIN.name
})

ix.config.Add("safeboxOpenTime", 0.4, "How long it takes to open a safebox.", nil, {
...
function PLUGIN:InitializedPlugins()
	ix.inventory.Register("safebox", ix.config.Get("safeInvWidth"), ix.config.Get("safeInvHeight"), false)
end
