...
function PLUGIN:PlayerSpawn(ply)
  local FirstConnect = PLUGIN:HasPlayerFlags(ply,"A")
  if FirstConnect != true then
    print("Given "..ply:SteamName().." starter flags")
    PLUGIN:GivePlayerFlags(ply,"ABV")
    ix.chat.Send(ply, "FirstConnectReg", ply:SteamName())
  end 
end

ix.chat.Register("FirstConnectReg", {
        OnChatAdd = function(self, speaker, text)
            chat.AddText(Color(74, 101, 255), ""..text.." has connected for the first time!")
        end,
		CanHear = function(self, speaker, listener) return true end,
        description = "FrstCnnctReg",
        indicator = "chatPerforming",
        deadCanChat = true,
})

do
  ix.flag.Add("A", "You've joined the server before.", function(client, isGiven)
  		if (isGiven) then
  ...
  ix.flag.Add("B", "Build some cool stuff!", function(client, isGiven)
  		if (isGiven) then
  ...
  ix.flag.Add("V", "Sometimes you just need to drive.", function(client, bGiven)
  end)
