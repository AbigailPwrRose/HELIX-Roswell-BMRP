...
function SWEP:SecondaryAttack()
	...
	if (SERVER and IsValid(entity)) then
		...
		elseif (entity:IsPlayer() and entity:Health() <= 75) then
            self.Owner:EmitSound("Weapon_StunStick.Melee_HitWorld")
        ...
        elseif (IsValid(entity)) then
			local damageInfo = DamageInfo()
				damageInfo:SetAttacker(self.Owner)
				damageInfo:SetInflictor(self)
				damageInfo:SetDamage(self.Secondarty.Damage)
				damageInfo:SetDamageType(DMG_CLUB)
				damageInfo:SetDamagePosition(trace.HitPos)
				damageInfo:SetDamageForce(self.Owner:GetAimVector() * 10000)
			entity:DispatchTraceAttack(damageInfo, data.start, data.endpos)
            self.Owner:Notify("Target HP too high to stun!")
		else
